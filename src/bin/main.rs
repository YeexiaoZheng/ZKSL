...
use zkml::{graph::Graph, model::ModelCircuit, numerics::numeric::NumericConfig, utils::{helpers::{to_field, NUMERIC_CONFIG}, loader::load_from_json}};
...
fn main() {
    ...
    *nconfig.lock().unwrap() = NumericConfig {
    ...
    let output = circuit.forward().iter().map(|x| to_field(*x)).collect::<Vec<_>>();
