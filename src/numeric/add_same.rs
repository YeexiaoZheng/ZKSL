...
type AddSameConfig = NumericConfig;

pub struct AddSameLayouter<F: PrimeField> {
    pub config: Rc<AddSameConfig>,
    _marker: PhantomData<F>,
}

impl<F: PrimeField> AddSameLayouter<F> {
    ...
    pub fn configure(
        ...
        /* [2, 3, 4, 5] + 2 = [4, 5, 6, 7]
            ...
            4  5   6   7   00  00 |  0
        */
        meta.create_gate("add same gate", |meta| {
            let s = meta.query_selector(selector);
            let add = meta.query_advice(columns[columns.len() - 1], Rotation::cur());
            (0..(columns.len() - 1))
                .into_iter()
                .map(|i| {
                    let inp = meta.query_advice(columns[i], Rotation::cur());
                    let outp = meta.query_advice(columns[i], Rotation::next());
                    s.clone() * (inp + add.clone() - outp)
                })
                .collect::<Vec<_>>()
        });

        let mut selectors = numeric_config.selectors;
        selectors.insert(NumericType::AddSame, selector);
...
impl<F: PrimeField> NumericLayout<F> for AddSameLayouter<F> {
    fn name(&self) -> String {
        "AddSame".to_string()
    }
    ...
    fn layout_unit(
        ...
        // Assign add
        add.copy_advice(|| "", region, columns[columns.len() - 1], row_offset)?;

        // Enable the selector
        if self.config.use_selectors {
            let selector = self.config.selectors.get(&NumericType::AddSame).unwrap();
        ...
        Ok(inp
            ...
            .map(|(idx, inp)| {
                ...
                let res = inp.value().copied() + add.value().copied();
                region
