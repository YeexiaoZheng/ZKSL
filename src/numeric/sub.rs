...
type SubConfig = NumericConfig;

pub struct SubLayouter<F: PrimeField> {
    pub config: Rc<SubConfig>,
    _marker: PhantomData<F>,
}

impl<F: PrimeField> SubLayouter<F> {
    ...
    pub fn configure(meta: &mut ConstraintSystem<F>, numeric_config: NumericConfig) -> SubConfig {
        ...
        /* [5, 6, 7, 8] - [1, 2, 3, 4] = [4, 4, 4, 4]
           ...
           4  4   4   4   00   |  0
        */
        meta.create_gate("sub gate", |meta| {
            ...
            (0..columns.len())
                ...
                .map(|i| {
                    ...
                    s.clone() * (outp + inp2 - inp1)
                })
                .collect::<Vec<_>>()
        });

        let mut selectors = numeric_config.selectors;
        selectors.insert(NumericType::Sub, selector);
...
impl<F: PrimeField> NumericLayout<F> for SubLayouter<F> {
    fn name(&self) -> String {
        "Mul".to_string()
    }
    ...
    fn layout_unit(
        ...
        if self.config.use_selectors {
            let selector = self.config.selectors.get(&NumericType::Sub).unwrap();
...
mod tests {
    ...
    #[derive(Clone, Debug)]
    pub struct SubLayerConfig<F: PrimeField> {
        pub numeric_config: Rc<NumericConfig>,
        pub public: Column<Instance>,
        pub _marker: PhantomData<F>,
    }

    pub struct SubLayerCircuit<F: PrimeField> {
        pub input1: FieldTensor<F>,
        pub input2: FieldTensor<F>,
    }

    impl<F: PrimeField> SubLayerCircuit<F> {
        pub fn construct(input1: FieldTensor<F>, input2: FieldTensor<F>) -> Self {
            Self { input1, input2 }
        }
    }

    impl<F: PrimeField> Assign<F> for SubLayerCircuit<F> {}

    impl<F: PrimeField> Circuit<F> for SubLayerCircuit<F> {
        type Config = SubLayerConfig<F>;
        ...
        fn configure(meta: &mut ConstraintSystem<F>) -> Self::Config {
            ...
            let numeric_config = SubLayouter::<F>::configure(meta, numeric_config);
    ...
    fn test_sub_circuit() {
        ...
        let expected_output: Vec<Int> = vec![-1, -1, -1, -1, -1, -1, -1, -1];
        ...
        // 构造电路实例
        let circuit = SubLayerCircuit::construct(input1, input2);
