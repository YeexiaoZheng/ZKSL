...
impl<F: PrimeField> DivSameLayouter<F> {
    ...
    pub fn configure(
        ...
        /* [8, 7, 6, 5] / 2 = [4, 3, 3, 2]
            ...
            -----------------------
            inp        d|   0
            reminder   f|   1           // f means symbol of div, value of 1 or -1
            output      |   0
        ...
        */
        // Assume that a / b = c + r
        meta.create_gate("div same", |meta| {
            (0..columns.len() - 1)
                .map(|i| {
                    let s = meta.query_selector(selector);
                    let a = meta.query_advice(columns[i], Rotation::prev());
                    let b = meta.query_advice(columns[columns.len() - 1], Rotation::prev());
        ...
        meta.lookup("div same lookup0", |meta| {
            let s = meta.query_selector(selector);
            let b = meta.query_advice(columns[columns.len() - 1], Rotation::prev());
            let f = meta.query_advice(columns[columns.len() - 1], Rotation::cur());
            vec![(s.clone() * (b * f - one.clone()), natural_lookup)]
        });
        for i in 0..(columns.len() - 1) {
            // Check |b| - |r| > 0
            // We know b * f > 0
            // So we only need to check b * f - r > 0 and b * f + r > 0
            // and b, r is integer,so we check b * f - r - 1 >= 0 and b * f + r - 1 >= 0
            meta.lookup("div same lookup1", |meta| {
                let s = meta.query_selector(selector);
                let b = meta.query_advice(columns[columns.len() - 1], Rotation::prev());
                let f = meta.query_advice(columns[columns.len() - 1], Rotation::cur());
                let r = meta.query_advice(columns[i], Rotation::cur());
                vec![(s.clone() * (b * f - r - one.clone()), natural_lookup)]
            });
    ...
    fn layout_unit(
        ...
        let _ = inp
            ...
            .map(|(idx, inp)| {
                ...
                let res = inp.value().zip(div.value()).map(|(a, b)| div_mod(a, b).1);
                region
        ...
        let res = inp
            ...
            .enumerate()
            .map(|(idx, inp)| {
                let res = inp.value().zip(div.value()).map(|(a, b)| div_mod(a, b).0);
                region
...
mod tests {
    ...
    use crate::{
        numeric::{
            div_same::DivSameLayouter, nonlinear::natural::NaturalLookUp, NumericConfig,
    ...
    fn test_div_same_circuit() {
        ...
        let expected_output = input1.iter().map(|a| a / b).collect::<Vec<_>>();
