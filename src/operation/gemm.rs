...
use crate::{
    numeric::{div::DivChip, dot::DotChip, Numeric, NumericConfig, NumericConsumer, NumericType},
    utils::{
...
impl<F: PrimeField> GemmChip<F> {
    ...
    pub fn backward(
        ...
        Ok(vec![
            inpgrad.dot(&weight.t()).into_dyn() / numeric_config.scale_factor as Int,
            input.t().dot(&inpgrad).into_dyn() / numeric_config.scale_factor as Int,
        ])
    }
}
