...
pub struct MulChip<F: PrimeField> {
...
impl<F: PrimeField> Numeric<F> for MulChip<F> {
    fn name(&self) -> String {
        "Mul".to_string()
    }
    ...
    fn compute_row(
        ...
        if self.config.use_selectors {
            let selector = self.config.selectors.get(&NumericType::Mul).unwrap()[0];
