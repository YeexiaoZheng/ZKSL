...
pub struct SubChip<F: PrimeField> {
    pub config: Rc<SubConfig>,
    _marker: PhantomData<F>,
}

impl<F: PrimeField> SubChip<F> {
    ...
    pub fn configure(meta: &mut ConstraintSystem<F>, numeric_config: NumericConfig) -> SubConfig {
        ...
        selectors.insert(NumericType::Sub, vec![selector]);
...
impl<F: PrimeField> Numeric<F> for SubChip<F> {
    fn name(&self) -> String {
        "Sub".to_string()
    }
    ...
    fn compute_row(
        ...
        if self.config.use_selectors {
            let selector = self.config.selectors.get(&NumericType::Sub).unwrap()[0];
